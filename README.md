...
- **Grouped updates** (`Bump the <group> group with N updates`) are recognized and escalated explicitly — the title carries no per-dependency semver levels, so no guessing. Multi-dependency titles (`bump A from … to … and B from … to …`) are parsed with the **highest** level applied. Anything else escalates as `unknown`
- Diffs beyond 500 changed files (5 pages × 100) are truncated
